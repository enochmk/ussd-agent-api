...
const clearExpiredSession = async () => {
  const timestamp = moment().subtract(CLEAR_EXPIRY_VALUE, CLEAR_EXPIRY_UNIT).format();
  ...
  Logger(`clearExpiredSession|Older than: ${CLEAR_EXPIRY_VALUE} ${CLEAR_EXPIRY_UNIT}|Timer Interval: ${CLEAR_EXPIRY_INTERVAL} seconds|${timestamp}|${response.rowsAffected[0]}|${stmt}`);
}
