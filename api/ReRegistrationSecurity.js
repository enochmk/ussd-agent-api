...
// * call the security re-registration API
const ReRegistrationSecurity = async (data, requestID) => {
	Logger(`${requestID}|SecurityAPI|request|${data}`);

	try {
		const response = await axios.post(
			process.env.NON_BIO_RE_REGISTRATION_SECURITY_URL,
			data
		);

		Logger(`${requestID}|SecurityAPI|success|${JSON.stringify(response.data)}`);
		...
		await SMS(data.msisdn, error.response.data.message, requestID);
